...
internal static class PartnerEndpoints
{
    public static void MapPartnerEndpoints(this IEndpointRouteBuilder endpoints)
    {
        ...
        partners.MapGet(string.Empty, ListAsync).RequireAuthorization(PortfolioOnboardingPermissions.Read);
        partners.MapPost(string.Empty, CreateAsync).RequireAuthorization(PortfolioOnboardingPermissions.Write);
        partners.MapGet("/{partnerId:guid}", GetAsync).RequireAuthorization(PortfolioOnboardingPermissions.Read);
        partners.MapPatch("/{partnerId:guid}", UpdateAsync).RequireAuthorization(PortfolioOnboardingPermissions.Write);
    }
