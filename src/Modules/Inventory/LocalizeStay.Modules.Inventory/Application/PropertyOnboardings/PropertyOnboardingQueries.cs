...
internal sealed record ReadinessGateResponse(Guid Id, string Type, string Status, string? Notes, DateTimeOffset? ValidatedAt, string? ValidatedBy, DateTimeOffset UpdatedAt);
internal sealed record PendingIssueResponse(Guid Id, string Description, string OwnerType, string? AssigneeId, string Status, string? RelatedGateType, DateTimeOffset? TargetAt, DateTimeOffset OpenedAt, DateTimeOffset? ResolvedAt);
...
internal static class PropertyOnboardingMapper
{
    ...
    internal static PropertyOnboardingResponse ToResponse(PropertyOnboarding item, IReadOnlyList<DuplicateCandidateResponse>? candidates = null) => new(item.Id, item.PartnerId, item.PreselectionId, new PropertyResponse(item.Property.Name, item.Property.DestinationId, new AddressResponse(item.Property.Address.Street, item.Property.Address.Number, item.Property.Address.Complement, item.Property.Address.District, item.Property.Address.City, item.Property.Address.State, item.Property.Address.PostalCode, item.Property.Address.CountryCode)), ContractValue(item.LifecycleStatus), ContractValue(item.ReadinessStatus), item.ReadinessGates.Select(g => new ReadinessGateResponse(g.Id, ContractValue(g.Type), ContractValue(g.Status), g.Notes, g.ValidatedAt, g.ValidatedBy, g.UpdatedAt)).ToList(), item.PendingIssues.Select(i => new PendingIssueResponse(i.Id, i.Description, ContractValue(i.OwnerType), i.AssigneeId, ContractValue(i.Status), i.RelatedGateType is null ? null : ContractValue(i.RelatedGateType.Value), i.TargetAt, i.OpenedAt, i.ResolvedAt)).ToList(), new DuplicateReviewStateResponse(item.DuplicateReviewRequiresDecision, candidates ?? [], item.DuplicateReviews.OrderByDescending(review => review.ReviewedAt).Select(review => ContractValue(review.Decision)).FirstOrDefault()), ToBlockingReasons(item), item.OpenedAt, item.TargetSubmissionAt, item.SubmittedAt, item.ClosedAt, item.CloseReason, item.CreatedAt, item.UpdatedAt);
