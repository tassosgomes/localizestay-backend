#!/usr/bin/env bash
#
# Gate deterministico do AI Flow.
#
# Roda formatacao, build e testes SEM consumir contexto de LLM: o `ai-flow-validator`
...
#
# Uso:
#   scripts/ai-flow/gate.sh [--filter=<expr>]... [--sln=<path>] [--skip-tests]
#
...
# ---------------------------------------------------------------------------
# Escopo: arquivos alterados desde o ultimo commit (= desde o checkpoint da task)
# ---------------------------------------------------------------------------
mapfile -t CHANGED < <(
...
# ---------------------------------------------------------------------------
TEST_STATUS="pulado"
if ((SKIP_TESTS == 0)) && ((${#FILTERS[@]} > 0)); then
  RESULTS=()
  for f in "${FILTERS[@]}"; do
...
# 4. Higiene do diff
# ---------------------------------------------------------------------------
if ! run git diff --check; then
  fail "diff-check" "git diff --check" "$OUT"
fi
